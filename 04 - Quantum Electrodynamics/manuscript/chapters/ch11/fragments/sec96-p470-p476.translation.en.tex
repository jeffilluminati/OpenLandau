...
We see that the corrections to the Born formulas for the integral cross sections of bremsstrahlung and pair production are given by one and the same function $f(\alpha Z)$.

Formula (96.24), not connected with any restrictions on the magnitude of $Z\alpha$, permits passage to the classical limit $\hbar \to 0$, $Z\alpha \to \infty$. In this limit one must also set $\varepsilon \approx \varepsilon'$. Bearing in mind the asymptotic expression $\Psi(z) \approx \ln z$ for $|z| \to \infty$ and the value $\Psi(1) = -C$ ($C$ is the Euler constant), we find for the effective retardation
\begin{equation}
\hbar\omega\,d\sigma = \frac{16Z^2 r_\mathrm{e}^2 e^2}{3c}\!\left[\ln\frac{2\varepsilon^2}{mc\omega Ze^2} - \frac{1}{2} - C\right]d\omega.
\label{eq:96.25}
\end{equation}
This expression, not containing $\hbar$, is the classical spectral distribution of the intensity of bremsstrahlung.
