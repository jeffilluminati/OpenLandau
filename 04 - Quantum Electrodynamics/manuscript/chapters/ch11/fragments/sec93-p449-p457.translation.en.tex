...
When passing through a medium containing $N$ atoms per unit volume, a fast electron loses on average its energy to bremsstrahlung

at the rate
\[
-\frac{d\varepsilon}{dx} = N\varepsilon\varkappa_\text{rad}/\varepsilon,
\]
where $\varkappa_\text{rad}/\varepsilon$ is the cross section of energy loss. For large $\varepsilon$ this ratio grows logarithmically (in the unscreened case). Under complete screening it tends to a constant, $\approx 4Z^2\alpha r_\mathrm{e}^2\ln(1/(\alpha Z^{1/3}))$.
