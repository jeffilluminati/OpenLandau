...
\begin{tikzpicture}[x=1.15cm,y=3.1cm]
  \draw[step=0.5cm,gray!55,very thin] (1,0) grid (5,1.1);
...
\begin{equation}
 ...
 \tag{138.12}
\end{equation}
The word ``constant'' is in quotation marks because $\kappa$ is actually the
first term in an expansion in powers of the small quantity $ka$ of a function
depending on the properties of the short-range forces. As noted in \S~133,
low-energy resonance corresponds to an anomalously small value of $\kappa$.
For better accuracy the next term, of order $k^2$, must therefore also be
included. Its coefficient has a ``normal'' order of magnitude; thus in
(138.12), $-\kappa$ must be replaced by\footnote{For proton--proton
scattering, the constants $a=1/\kappa_0$ and $r_0$ have the values
$a=-7.8\cdot10^{-13}$ cm and $r_0=2.8\cdot10^{-13}$ cm (the Coulomb unit of
length is $2\hbar^2/m_pe^2=57.6\cdot10^{-13}$ cm). These values refer to a
pair of protons with antiparallel spins; by the Pauli principle, two protons
with parallel spins cannot be in an $s$-state at all.}
\[
 -\kappa_0+\frac12r_0k^2.
\]

As indicated in \S~133, resonance may be associated with either a true or a
virtual

\SourcePage{693}{696}
discrete bound state of the system. It can be shown\footnote{See
L.~D.~Landau and Ya.~A.~Smorodinsky, ZhETF, 1944, Vol.~14, p.~269.} that the
sign of $\kappa$ remains the criterion distinguishing a true level from a
virtual one.
