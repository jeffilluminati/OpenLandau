...
\SourcePage{253}{256}
where\footnote{When matrices are written as in (55.7), rows and columns are
numbered by the values of $\sigma$: the row number corresponds to the first
index of a matrix element and the column number to the second. Here the
numbers are $+1/2,-1/2$. According to (55.4), the operator acts by multiplying
the $\sigma$th row of the matrix by the wave-function components arranged in
the column $\displaystyle\psi=\binom{\psi(1/2)}{\psi(-1/2)}$.}
\begin{equation}
