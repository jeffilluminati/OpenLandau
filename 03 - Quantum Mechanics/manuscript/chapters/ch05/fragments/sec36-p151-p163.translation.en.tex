...
\begin{tikzpicture}[x=2.2cm,y=1.1cm,>=stealth]
 ...
 \fill (0,0) circle (1.2pt) node[right=4pt,yshift=2pt] {$\xi=0$};
 \fill (0,1) circle (1.2pt) node[below right=3pt] {$\xi=2ikr$};
 \draw[thick,->] (-0.35,-0.18)--(0.82,-0.18);
 \draw[thick,postaction={decorate},decoration={markings,
   mark=at position .50 with {\arrow{>}}}]
   (0.82,-0.18) arc (-90:90:0.43 and 0.59);
 \draw[thick,->] (0.82,1)--(-0.35,1);
\end{tikzpicture}
...
\begin{tikzpicture}[x=1.2cm,y=0.8cm,>=stealth]
 \draw[->] (0,-1.1)--(0,2.2) node[above] {$U(r)$};
 \draw[->] (0,0)--(4,0) node[right] {$r$};
 \draw[thick,domain=0.42:4,samples=100,smooth]
...
\begin{center}
\begin{tikzpicture}[x=1.2cm,y=0.8cm,>=stealth]
 \draw[->] (0,0)--(0,3.2) node[above] {$U(r)$};
 \draw[->] (0,0)--(4,0) node[right] {$r$};
 \draw[thick,domain=0.42:3.48,samples=100,smooth]
   plot (\x,{0.55/(\x*\x)+0.26*\x*\x});
\end{tikzpicture}
