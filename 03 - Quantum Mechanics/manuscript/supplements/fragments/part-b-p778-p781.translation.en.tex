...
\begin{tikzpicture}[scale=1.05,>=stealth,line cap=round,line join=round]
  ...
  \draw[->,thin] (0,.72) arc[start angle=90,end angle=38,radius=.72];
  \node at (.38,.62) {$\frac{\pi}{3}$};
  \draw[->,thin] (.55,.57) arc[start angle=-90,end angle=-45,radius=.35];
  \node at (.78,.65) {$\frac{\pi}{4}$};
  \draw[->,thin] (.51,-.29) arc[start angle=-30,end angle=-90,radius=.5];
